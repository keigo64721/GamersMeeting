    ...
    @section('content')
        ...
        <dev class="prof-contents">
            ...
            <dev class="right-side">
                ...
                <p class="cont-age">年齢 : {{ $auth->status->age }}歳</p>
                <p class="cont-sex">性別: {{ $auth->status->sex }}</p>
                <p class="cont-game">ゲーム名 : {{ $auth->status->game->name }}</p>
                <p class="cont-playstyle">プレイスタイル :{{ $playstyle[(int)$auth->status->playstyle] }}</p>
                <div class="playwith">
